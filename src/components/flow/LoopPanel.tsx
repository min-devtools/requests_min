...
// Lives in the right dock as the Step detail tab for loop blocks — no editor; just the pass
// count (edit via the same modal as the block's pencil) and the in/out anchor legend.
  ...
  return (
    <section className="flow-node-panel flow-loop-panel" aria-label="Selected loop step">
      ...
      <div className="flow-loop-body">
        <div className="flow-loop-passes">
          <span>Body passes</span>
          <strong>×{node.config.count}</strong>
          {step?.remaining != null && (
          ...
          <button
            ...
            className="tool-btn"
            disabled={ft.running}
            onClick={() => void editLoopNode(tabId, node.id)}
          >
