...
`{{$name}}` is a dynamic built-in (Postman-style: `$uuid`, `$timestamp`, `$randomEmail`, …) — a fresh value per occurrence, generated by the `fake` crate; the `$` namespace is reserved (env/secrets can't shadow it). Registry: `src-tauri/src/dynamic.rs`, UI catalog: `src/lib/dynamicVariables.ts`.
...
src-tauri/src/
  ...
  dynamic.rs      {{$name}} dynamic variables (uuid/timestamps/faker data)
